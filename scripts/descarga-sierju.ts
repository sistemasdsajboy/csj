...
const PASSWORD = "";
const URL =
  "https://sistemaestadistico.ramajudicial.gov.co/Sierju-Web/app/login";
...
async function irAPaginaDescarga(page) {
  const linkVisible = await page
    .getByText("Reporte Actividad Diligenciamiento")
    .isVisible();
  if (!linkVisible) {
...
async function descargarDatosDespachoSierju(periodo, codigoDespacho) {
  ...
  await page.fill(
    "[name='formReporteFuncionario:inputFunDespacho']",
    codigoDespacho
  );
  await page.fill(
    "[name='formReporteFuncionario:fechaInicio_input']",
    `01/01/${periodo}`
  );
  await page.fill(
    "[name='formReporteFuncionario:fechaFin_input']",
    `31/12/${periodo}`
  );
  ...
  const textoSinResultados = await page
    .getByText("No hay Resultados para mostrar")
    .isVisible();
  ...
  const filas = await page.$$(
    "[id='formReporteFuncionario:tablerReporteInformes_data'] tr"
  );
  ...
  for await (const fila of datosFilasParaExportar) {
    ...
    const enlaceDescarga = await page.$(
      "[id='formFormulariosRecuperar:j_idt102']"
    );
